...
<%
  //List<MemHobbyVO> list = (List<MemHobbyVO>)request.getAttribute("list");
%>
<jsp:useBean id="list" scope="session" type="java.util.List<MemHobbyVO>"/>
<!doctype html>
...
<title>會員興趣資料 - listOneMemHobby.jsp</title>
...
<table id="table-1">
	<tr><td>
		 <h3>會員興趣資料 - ListSomeMemHobby.jsp</h3>
...
<table>
	...
	<c:forEach var="memHobbyVO" items="${list}" begin="<%=pageIndex%>" end="<%=pageIndex+rowsPerPage-1%>">
		<tr>
			<td>${memHobbyVO.mem_ID}</td>
			<td>${memHobbyVO.hobby_ID}</td>
