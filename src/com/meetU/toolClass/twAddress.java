...
@WebServlet("/twAddress")
public class twAddress extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		BufferedReader br = new BufferedReader(new InputStreamReader(getServletContext().getResourceAsStream("/FrontEnd/meetup/img/streetname.txt"),"UTF-8"));
		...
		try {
			...
			for (int i = 0; i < jArray.length(); i++) {
				...
				for (int j = 0; j < area.length(); j++) {
					...
					String areaName = jobj.getString("ZipCode")+jobj.getString("AreaName");
...
//				System.out.println(areaMap.size());   共有24個縣市
			}
